  ...
  describe 'without attributes' do
    before do
      stub_class('DummyEmptyHashType') do
        extend Qwack::BaseHash # rubocop:disable RSpec/DescribedClass
      end
    end
  ...
  describe 'with a simple attribute' do
    ...
    describe '#validation_errors' do
      ...
      it 'returns validation errors of Hash items from the attribute type' do
        allow(DummyType).to receive(:validation_errors)
          .with(1, 'root.key').once.and_return(%w[error1 error2])
        expect(DummySimpleHashType.validation_errors({ 'key' => 1 }))
          .to eq(%w[error1 error2])
      end
    end

    describe '#mock' do
      it 'forwards mocking to the attribute type' do
        allow(DummyType).to receive(:mock)
          .with(nil).once.and_return(1)
        expect(DummySimpleHashType.mock)
          .to eq({ 'key' => 1 })
      end

      it 'forwards input to the attribute type' do
        allow(DummyType).to receive(:mock)
          .with(2).once.and_return(2)
        expect(DummySimpleHashType.mock({ 'key' => 2 }))
          .to eq({ 'key' => 2 })
      end
    end
  end
